...
static void gyro_value_get(struct work_struct *work)
{
	...
	for(i=0; i < len; i++) {
		...
		data.x += dev->data[dev->write_p].x;
		data.y += dev->data[dev->write_p].y;
		data.z += dev->data[dev->write_p].z;
		//gyro_info("---- %d, %d, %d .\n", data.x*35/400000, data.y*35/400000, data.z*35/400000);
